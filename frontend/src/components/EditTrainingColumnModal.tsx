...
import type { TrainingColumn, UpdateTrainingColumnRequest } from '../types';

interface EditTrainingColumnModalProps {
  column: TrainingColumn;
  onClose: () => void;
  onSuccess: (updated: TrainingColumn) => void;
}

const EditTrainingColumnModal: React.FC<EditTrainingColumnModalProps> = ({ column, onClose, onSuccess }) => {
  const [formData, setFormData] = useState<UpdateTrainingColumnRequest>({
    title: column.title,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await columnApi.update(column.id, formData);
      onSuccess(updated);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to update training column');
    } finally {
      setLoading(false);
    }
  };
...
export default EditTrainingColumnModal;
